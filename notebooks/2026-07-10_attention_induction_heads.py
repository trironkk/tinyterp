...
# %% [A] Setup: load the trained 27.5M checkpoint and the v2048 tokenizer from the shared cache
import torch
...
# %% [B] Capture helper: enable store_attn on all blocks, forward once, stack per-layer weights
...
# %% [C] Attention census: one 8x8 grid of per-head heatmaps on the Paris prompt
...
# %% [D] Labeled zoom-ins: the top head for each of three motifs, chosen by score
...
# %% [E] Motif census: previous-token, attend-to-self, sink scores over a batch of real windows
...
# %% [F] Induction census: score every head on repeated-random-token sequences, then rank
REP, N_SEQ = 64, 8
...
# %% [G] Visualize the top induction head's off-diagonal stripe on one repeated sequence
demo = repeated[:1]
with torch.no_grad():
    model(demo)
...
# %% [H] Ablate a head concretely: next-token probability and greedy generation, per ablated head
import math
...
# %% [I] Ablate each head over 32 random samples; sort by language-modeling damage
...
# %% [J] Prune least-significant-first: loss curve, then watch generation drift, deviations in red
