...
# %% [A] Setup: load the v2048 tokenizer (train and cache it if absent), select a device
...
# %% [B] Config and model assembly: weights and plumbing here, attention forward in [C]
...
#   no information yet.
# %% [D] Forward-pass verification: shapes, parameter count, causality, softmax
...
# %% [E] Data pipeline: sample articles, encode, train/val split, get_batch
import random
...
# %% [F] Overfit one batch: sanity-check the forward and backward wiring
...
#   address.
# %% [G] Training loop: train on the sample, track train and val loss
import time
...
#   retrained casually.
# %% [I] Scale up: rerun the experiment across scales with early stopping, compare
import copy
...
# %% [J] Autoregressive sampling: generate from a prompt
@torch.no_grad()
...
#   (transformer_simplewiki_<config-hash>.pt: state_dict, config, loss history, samples). This cell
...
# %% [K] Large-scale run: load the ~1h full-corpus checkpoint and inspect it
...
# %% [M] Persist and inventory: the artifacts this notebook depends on
